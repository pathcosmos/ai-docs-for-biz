...
```mermaid
flowchart LR
    A[STEP 1<br/>데이터 통합·정형화<br/>ICS/MES/CMMS] --> B[STEP 2<br/>AI 엔진 4 종 구축<br/>5.2-a + 5.2-b + 5.2-d + 5.2-f]
    B --> C[STEP 3<br/>현장 통합 어시스턴트<br/>HMI · 태블릿 · 정비 모바일]
    C -.피드백.-> A
```
...
```mermaid
flowchart LR
    A[AI 추천·예측<br/>+ 근거 표시] --> B[작업자 검토 UI<br/>3단 평가 + 사유]
    B --> C{평가 결과}
    C -->|사용 가능| D[운영 적용<br/>+ 신뢰 신호 누적]
    C -->|수정 필요| E[작업자 보정<br/>+ 미세 조정 라벨]
    C -->|부적합| F[학습 강화 큐<br/>+ 우선 재학습]
    D --> G[결과 피쳐 자동 편입<br/>플라이휠]
    E --> G
    F --> G
    G --> H[Track 2 SCN-MLO-03<br/>피드백 루프]
    B --> I[자유 메모<br/>LLM 태깅]
    I --> J[Track 3 RAG 인덱스<br/>노하우 자산화]
```
...
```mermaid
flowchart LR
    subgraph OT[OT 구역 — 기존]
        P[ICS PLC<br/>1ZHM/2ZHM/정밀압연]
        H[ICS Historian]
        M[MES / QMS / ERP]
        C[CMMS 자유 텍스트]
        F[FEMS]
    end
    subgraph IT[IT 구역 — 신규 AI 레이어]
        T[TSDB · RDB Replica<br/>· Vector DB · Object Storage]
        E[AI 엔진 4종<br/>5.2-a/b/d/f]
        R[모델 레지스트리<br/>+ 피쳐 스토어]
        D[모니터링·드리프트<br/>대시보드]
    end
    subgraph UX[현장 UX]
        U1[MES 작업지시 + AI 추천]
...
```mermaid
gantt
    title [고객사] 제조AI 통합 플랫폼 구축 일정 (18 개월 가정)
    dateFormat  YYYY-MM
    axisFormat  %m/%y

    section 인프라
    데이터 통합·정형화 (§5.1)            :done, p1-1, 2026-05, 3M
    MLOps 기반 구축 (Track 2 §5.1~5.4)    :p1-2, 2026-06, 4M
    HITL UI · 피드백 루프 (SCN-MLO-03)     :p1-3, 2026-08, 4M
    RAG 인프라 (Vector DB · 권한)          :p2-0, 2026-09, 3M

    section Phase 1 주력
    STL-04 패스 스케줄 (5.2-a) — PoC      :p1-4, 2026-07, 3M
    STL-04 — Pilot                       :p1-5, 2026-10, 3M
    STL-05 두께 예측 (5.2-b) — PoC        :p1-6, 2026-08, 3M
    STL-05 — Pilot                       :p1-7, 2026-11, 3M

    section Phase 2 확장
    STL-09 예지보전 (5.2-d) — IoT 증설    :p2-1, 2026-09, 4M
    STL-09 — PoC + Pilot                 :p2-2, 2027-01, 4M
    STL-06 소둔 최적화 (5.2-e) — PoC      :p2-3, 2026-11, 3M
    STL-06 — Pilot                       :p2-4, 2027-02, 3M
    LLM-02 장애 RAG (5.2-f) — Pilot      :p2-5, 2026-12, 4M

    section Phase 3 안정화
    챔피언·챌린저 검증 (전 시나리오)        :p3-1, 2027-04, 3M
    KPI 검증 + 보고서                     :p3-2, 2027-06, 3M
    확산 로드맵 수립                      :p3-3, 2027-09, 2M
```
...
```mermaid
flowchart LR
    A[본 사업<br/>9~18 개월<br/>패키지 2 통합] --> B[Phase 후속 1<br/>+12 개월<br/>SCN-LLM-01 SOP RAG<br/>+ STL-08 밀시트 OCR]
    B --> C[Phase 후속 2<br/>+24 개월<br/>STL-10 표면 비전<br/>+ STL-11 UT 자동판정]
    C --> D[Phase 후속 3<br/>+36 개월<br/>SAF-01 안전 AI<br/>+ SAF-02 CBAM]
    D --> E[Phase 후속 4<br/>+48 개월<br/>산단 공동 연합학습<br/>UTL-01 에너지]
```
...
```mermaid
flowchart LR
    A[CMMS 장애 이력<br/>+ 설비 매뉴얼·도면<br/>+ KOSHA 가이드] --> B[수집 커넥터<br/>증분·이벤트]
    B --> C[파서·OCR<br/>PDF/HWP/DWG]
    C --> D[청킹·메타 추출<br/>권한 태깅]
    D --> E[임베딩<br/>멀티뷰]
    E --> F[Vector DB<br/>Weaviate]
    G[정비원 모바일·태블릿<br/>자연어 질의] --> H[하이브리드 검색<br/>Dense+BM25+메타]
    F --> H
    H --> I[Re-ranker]
    I --> J[LLM 생성<br/>근거 인용 강제]
    J --> K[응답·인용·부품번호<br/>모바일 UI]
    K --> L[정비원 평가<br/>👍/👎]
    L --> D
    K -.이상 알람 시 5.2-d 결합.-> M[과거 진동 프로파일·정비 사례]
```

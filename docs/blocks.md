...
```mermaid
graph LR
  A["1. 본 페이지에서<br/>15 블록 일람 확인"] --> B["2. 귀사 사업계획서<br/>§ 매칭 (위 표)"]
  B --> C["3. 해당 블록의<br/>전체 본문 link 클릭"]
  C --> D["4. 본문 + 삽화<br/>복사 (📋 hover 버튼)"]
  D --> E["5. 플레이스홀더<br/>일괄 치환"]
  E --> F["✅ 사업계획서 §3·4·5·6<br/>본문 완성"]

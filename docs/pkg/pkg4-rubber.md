...
flowchart LR
    subgraph 비정형_원천[비정형 · 이미지 원천]
        S1[배합표·SOP HWP/PDF<br/>부서별 산재 [수치]건]
        S2[과거 8D·5Why 보고서<br/>OEM 클레임 응대 이력]
        S3[MSDS·화관법 문서]
        S4[외관 검사 이미지<br/>다각도 [수치]TB]
        S5[교대 일지·검사 기록 수기]
    end
...
flowchart LR
    subgraph DATA[데이터 인프라]
        D1[배합 PLC<br/>토크·전력·온도]
        D2[압출 PLC + 레이저 치수<br/>+ 라인스캔]
        D3[가류 사이클 로그]
        D4[외관 검사 이미지]
        D5[8D 보고서·MSDS·SOP]
    end
    subgraph LGAI[LG AI 플랫폼 + EXAONE]
        L1[데이터레이크 단일 테넌시]
        L2[모델 레지스트리·피쳐 스토어]
        L3[EXAONE sLM<br/>한국어·도메인 어휘]
        L4[MLO-01 드리프트·재학습]
    end
    subgraph AI[AI 엔진 5종]
        E1[RUB-01 5.2-b<br/>배합 분산도]
        E2[RUB-02 5.2-b+c<br/>압출 치수+표면 결합]
        E3[RUB-05 5.2-c<br/>외관 검사]
        E4[LLM-03 5.2-f<br/>EXAONE 불량 보고서]
        E5[MLO-01<br/>드리프트·재학습]
    end
    subgraph UX[현장 UX — 단일 통합 UI]
        U1[배합사 모바일 + 토크 알람]
        U2[압출 작업자 + 치수·표면 알람]
        U3[검사원 검토 라벨링]
        U4[QA 8D 보고서 초안]
...
flowchart TD
    ...
    P5 --> C5[Retrieval + LLM<br/>EXAONE 분기]
    ...
    A --> E1[Stacking]
    A --> E2[Weighted Avg]
    A --> E3[Model Router<br/>RUB-02 5.2-b+c 결합]
    E1 --> O[운영 배포]
    E2 --> O
    E3 --> O
```
...
flowchart LR
    ...
    T --> R[모델 레지스트리<br/>LG AI 플랫폼]
    R --> B1[Edge 배포]
    R --> B2[Server 배포]
    B1 --> I[실시간 추론]
    B2 --> I
    I --> H[현장 HMI · MES 통합<br/>단일 통합 UI]
    H --> FB[운영 피드백<br/>품질·수율·HITL]
    FB --> P
    FB -.드리프트.-> T2[Track 2 MLOps<br/>SCN-MLO-01 1차 적용]
    L4 -.RAG.-> T3[Track 3 LLM+RAG<br/>SCN-LLM-03 + EXAONE]
```
...
```mermaid
flowchart LR
    subgraph OT[OT 구역 — [고객사]]
        P1[배합 믹서 PLC]
        P2[압출 PLC + 레이저 + 라인스캔]
        P3[가류기 사이클 로그]
        P4[외관 검사 카메라]
        E[ERP/MES]
        F[비정형 문서<br/>8D·SOP·MSDS]
    end
    subgraph LGAI[LG AI 플랫폼 — 신규 AI 레이어]
        T[데이터레이크<br/>TSDB·RDB·Object·Vector]
        EN[AI 엔진 5종]
        EX[EXAONE sLM]
        REG[모델 레지스트리·피쳐 스토어<br/>5종 도입]
        MON[MLO-01 드리프트·재학습]
    end
    subgraph UX[현장 UX — 단일 통합 UI 3 채널]
        U1[배합사·압출 작업자<br/>모바일 + HMI 알람]
        U2[검사원·QA<br/>PC 사이드 패널]
        U3[정비/운영팀<br/>드리프트 대시보드]
    end
    P1 --> T
    P2 --> T
    P3 --> T
...
```mermaid
gantt
    title [고객사] 자동차 부품 고무·폴리머 양산 AI 플랫폼 구축 일정 (12 개월 표준)
    dateFormat  YYYY-MM
    axisFormat  %m/%y

    section 인프라
    LG AI 플랫폼 데이터 통합 적재             :p1-1, 2026-05, 3M
    MLOps 5 종 (레지스트리·피쳐스토어·학습·서빙·모니터링) :p1-2, 2026-06, 4M
    HITL 단일 통합 UI 3 채널                  :p1-3, 2026-07, 4M

    section Phase 1 주력
    RUB-02 압출 5.2-b+c 결합 — PoC            :p1-4, 2026-06, 3M
    RUB-02 — Pilot                            :p1-5, 2026-09, 3M
    LLM-03 (EXAONE) 8D 보고서 — PoC           :p1-6, 2026-07, 3M
    LLM-03 — Pilot                            :p1-7, 2026-10, 3M

    section Phase 2 확장
    RUB-01 배합 분산도 — PoC·Pilot            :p2-1, 2026-10, 3M
    RUB-05 외관 검사 — PoC·Pilot              :p2-2, 2026-10, 3M
    MLO-01 드리프트 1차 적용 — 운영            :p2-3, 2026-09, 4M
    챔피언·챌린저 검증 + 재학습 트리거         :p2-4, 2027-02, 2M

    section Phase 2 정리
    KPI 검증 + OEM SQA 정합 + 후속 로드맵     :p2-5, 2027-04, 1M
```
...
```mermaid
flowchart LR
    A[본 사업<br/>12 개월 표준<br/>RUB-01·02·05·LLM-03 EXAONE·MLO-01] --> B[Phase 후속 1<br/>+12 개월<br/>RUB-05 라벨 대량 확장<br/>+ LLM-01 SOP RAG<br/>+ SAF-03 MSDS RAG]
    B --> C[Phase 후속 2<br/>+18 개월<br/>SAF-01 안전 영상 AI<br/>+ MLOps 풀 7 종<br/>+ LLM-04 도면 RAG]
    C --> D[Phase 후속 3<br/>+24 개월<br/>산단 공동 연합학습<br/>RUB-01·02·05 공동 모델<br/>대중소상생 확장]
```

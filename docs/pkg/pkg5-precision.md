...
flowchart LR
    subgraph 비정형_원천[비정형 · 이미지 원천]
        S1[CAD 도면 DWG·STEP·PDF<br/>부서별 산재 [수치]만건]
        S2[CAM 가공 프로그램]
        S3[공정설계 Excel]
        S4[작업표준서 HWP/PDF]
        S5[교대 일지 수기]
...
```mermaid
flowchart LR
    A[STEP 1<br/>클라우드 데이터 통합·정형화<br/>CNC/검사/전력/도면] --> B[STEP 2<br/>AI 엔진 4 종 구축<br/>5.2-b + 5.2-c + 5.2-e + 5.2-f]
    B --> C[STEP 3<br/>단일 통합 어시스턴트<br/>HMI · 모바일 · 검사실 PC]
    C -.피드백.-> A
```
...
flowchart LR
    subgraph 수집[데이터 수집]
        D1[CNC 컨트롤러 로그<br/>시계열]
        D2[ERP/검사 정형 DB]
        D3[3D 스캔·CMM 이미지]
        D4[CAD 도면<br/>DWG/STEP/PDF]
    end
    D1 --> L1[TSDB - 클라우드]
    D2 --> L2[RDB - 클라우드]
    D3 --> L3[Object Storage - 클라우드]
    D4 --> L4[Vector DB - 클라우드]
    L1 --> P[정제·라벨링]
    L2 --> P
    L3 --> P
    L4 --> P
    P --> F[피쳐 메타데이터<br/>경량 피쳐 테이블]
    F --> T[학습·평가<br/>베이스라인→후보→채택]
    T --> R[모델 레지스트리<br/>3종 압축 모드]
    R --> B1[Edge 배포 (MET-01)]
    R --> B2[Server 배포 (MET-03·UTL-01·LLM-04)]
    B1 --> I[실시간 추론]
    B2 --> I
    I --> H[단일 통합 UI<br/>SCN-MLO-03]
    H --> FB[운영 피드백<br/>품질·수율·HITL]
    FB --> P
    FB -.드리프트.-> T2[Track 2 MLOps<br/>SCN-MLO-01 압축 모드]
    L4 -.RAG.-> T3[Track 3 LLM+RAG<br/>SCN-LLM-04 한정]
```
...
flowchart LR
    subgraph OT[OT 구역 — 기존]
        P[CNC 컨트롤러<br/>선반/머시닝센터/MCT]
        Q[3D 스캐너·CMM]
        E[ERP - 단순 MES]
        L[전력량계]
        F[CAD 도면 파일서버]
    end
    subgraph CL[클라우드 SaaS — 신규 AI 레이어]
        T[TSDB · RDB · Vector DB · Object Storage<br/>단일 테넌시]
        EN[AI 엔진 4종<br/>5.2-b/c/e/f]
        R[모델 레지스트리·모니터링·피드백<br/>3종 압축 모드]
    end
...
```mermaid
gantt
    title [고객사] 정밀가공 클라우드 SaaS AI 플랫폼 구축 일정 (6 개월 압축)
    dateFormat  YYYY-MM
    axisFormat  %m/%y

    section 인프라
    클라우드 SaaS 데이터 적재 통합              :p1-1, 2026-05, 2M
    MLOps 압축 3 종 (레지스트리·모니터링·피드백)  :p1-2, 2026-06, 2M
    HITL 단일 통합 UI                            :p1-3, 2026-06, 2M

    section Phase 1 주력
    MET-01 CNC 공구 마모 — PoC                   :p1-4, 2026-05, 2M
    MET-01 — Pilot                              :p1-5, 2026-07, 2M
    MET-03 3D 치수 검사 — PoC                    :p1-6, 2026-06, 2M
    MET-03 — Pilot                              :p1-7, 2026-08, 2M

    section Phase 2 확장
    UTL-01 에너지 예측·시프트 — Pilot            :p2-1, 2026-08, 2M
    LLM-04 CAD 도면 검색 — Pilot                 :p2-2, 2026-08, 2M

    section Phase 2 정리
    KPI 검증 + 후속 로드맵 수립                   :p2-3, 2026-10, 1M
```
...
```mermaid
flowchart LR
    A[본 사업<br/>6 개월 압축<br/>MET-01·MET-03·UTL-01·LLM-04] --> B[Phase 후속 1<br/>+12 개월<br/>SCN-LLM-01 SOP RAG<br/>+ MET-03 전수 라벨링 확장]
    B --> C[Phase 후속 2<br/>+18 개월<br/>SCN-SAF-01 안전 영상 AI<br/>+ MLOps 풀 7 종]
    C --> D[Phase 후속 3<br/>+24 개월<br/>산단 공동 연합학습<br/>UTL-01·MET-01 공동 모델]
```
